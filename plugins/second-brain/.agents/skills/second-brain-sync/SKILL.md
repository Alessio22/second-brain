...
description: Sync this session's work into the second-brain knowledge base (session report, copied specs/plans, updated functional overview). Use when the user asks to sync second brain, record this session, or run second-brain-sync at a checkpoint.
---
...
git rev-parse HEAD 2>/dev/null || echo "none"   # current commit
...
- **Current branch**: from the "current branch" output in Step 0. If empty (detached HEAD), use `(detached HEAD at <short-sha>)`, where `<short-sha>` is the first 7 characters of the "current commit" from Step 0. This goes into the "Branch" line of Handoff.
