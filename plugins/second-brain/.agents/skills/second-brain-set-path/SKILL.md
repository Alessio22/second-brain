---
name: second-brain-set-path
description: Update the knowledge base path for this repo (or create the config if missing after a fresh clone). Use when the user asks to change the KB path, update second-brain-set-path, or set up the config on a freshly cloned repo.
---
...
```bash
cat .claude/second-brain.json 2>/dev/null || echo "MISSING"   # existing config
...
**Case B — config is `MISSING`**: go to step 2B.
...
Tell the user: "No `.claude/second-brain.json` found. Let's create one so you can use the second-brain-sync and second-brain-restore skills."
...
- If it does not exist, warn the user: "Folder `<knowledgeBasePath>` not found in the KB. Verify the path, or run the second-brain-init skill to initialize a new project from scratch."
...
You can now run the second-brain-restore skill to recover CLAUDE.md and memory.md from the KB.
```
