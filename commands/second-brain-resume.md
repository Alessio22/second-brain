...
Present the matching sessions as a list, one per line: date and topic (from the filename, e.g. `2026-06-12-keycloak-auth.md`), plus the status. Use the AskUserQuestion tool to let the user pick one (or let them say "skip" / pick none).
